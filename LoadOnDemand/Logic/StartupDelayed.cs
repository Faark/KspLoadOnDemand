    ...
    public class StartupDelayed: MonoBehaviour
    {
        ...
        public void Awake()
        {
            try
            {
                ...
                dumpManagedInternals();
                dumpManagedParts();

                calcSizeOfMissedImages();
#endif
            }
            catch (Exception err)
            {
        ...
        void calcSizeOfMissedImages()
        {
            ...
            foreach (var ti in GameDatabase.Instance.databaseTexture)
            {
                if (!Managers.TextureManager.IsManaged(ti))
                {
                    ...
                    {
                        ("MISSED TEXTURE WAS REMOVED: " + ti.name).Log();
                    }
                }
            }
