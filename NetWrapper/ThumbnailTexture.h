...
// A wapper for raw DX data... currently uncompressed.
...
public:
	...
	ThumbnailTexture(array<Byte>^ data, bool has_to_be_normal, TextureDebugInfo^ debug_info) :IAssignableTexture(debug_info){
		...
		{
			throw gcnew FormatException("Invalid data size. Expected " + expectedSize + " byte, got " + data->Length + " byte (both including a " + ByteDataHeaderOffset + " byte header)");
		}
...
public:
	...
	static BitmapFormat^ ConvertToBitmap(ThumbnailTexture^ source){
		...
		int pos = source->ByteDataHeaderOffset;
